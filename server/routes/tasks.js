...
function taskWithMeta(row) {
  const assignee = row.assignee_id ? db.prepare('SELECT id, name, avatar_color, email FROM users WHERE id = ?').get(row.assignee_id) : null;
  const commentCount = db.prepare('SELECT COUNT(*) as c FROM task_comments WHERE task_id = ?').get(row.id).c;
  const attachmentCount = db.prepare('SELECT COUNT(*) as c FROM task_attachments WHERE task_id = ?').get(row.id).c;
  return { ...row, assignee, commentCount, attachmentCount };
}

function attachmentMeta(row) {
  const uploader = row.uploaded_by ? db.prepare('SELECT id, name, avatar_color FROM users WHERE id = ?').get(row.uploaded_by) : null;
...
function register(router) {
  ...
  router.get('/api/tasks', async (req, res) => {
    ...
    if (projectId) {
      ...
      rows = db.prepare('SELECT * FROM tasks WHERE project_id = ? ORDER BY created_at DESC').all(Number(projectId));
    } else if (mine) {
      rows = db.prepare('SELECT * FROM tasks WHERE assignee_id = ? ORDER BY due_date IS NULL, due_date ASC').all(user.id);
    ...
    sendJSON(res, 200, { tasks: rows.map(taskWithMeta) });
  });
  ...
  router.post('/api/tasks', async (req, res) => {
    ...
    if (assigneeId) {
      const member = db.prepare('SELECT id FROM project_members WHERE project_id = ? AND user_id = ?').get(projectId, assigneeId);
    ...
    sendJSON(res, 201, { task: taskWithMeta(created) });
  });
  ...
  router.get('/api/tasks/:id', async (req, res) => {
    ...
    if (!canAccessProject(user, project)) return sendJSON(res, 403, { error: 'You do not have access to this task.' });
    ...
    `).all(task.id);
    const attachments = db.prepare('SELECT * FROM task_attachments WHERE task_id = ? ORDER BY created_at DESC').all(task.id).map(attachmentMeta);
    sendJSON(res, 200, { task: taskWithMeta(task), comments, attachments, project: { id: project.id, name: project.name } });
  });
  ...
  router.put('/api/tasks/:id', async (req, res) => {
    ...
    if (!managerLevel) {
      ...
      if (body.assigneeId !== undefined) {
        ...
        if (assigneeId) {
          const member = db.prepare('SELECT id FROM project_members WHERE project_id = ? AND user_id = ?').get(task.project_id, assigneeId);
          if (!member) return sendJSON(res, 400, { error: 'Assignee must be a member of this project.' });
        }
    ...
    sendJSON(res, 200, { task: taskWithMeta(updated) });
  });
  ...
  router.post('/api/tasks/:id/comments', async (req, res) => {
    ...
    const info = db.prepare('INSERT INTO task_comments (task_id, user_id, body) VALUES (?, ?, ?)').run(task.id, user.id, text);
  ...
  router.post('/api/tasks/:id/attachments', async (req, res) => {
    ...
    const created = db.prepare('SELECT * FROM task_attachments WHERE id = ?').get(info.lastInsertRowid);
    sendJSON(res, 201, { attachment: attachmentMeta(created) });
  });
  ...
  router.get('/api/tasks/:id/attachments/:attachmentId/download', async (req, res) => {
    ...
    const project = db.prepare('SELECT * FROM projects WHERE id = ?').get(task.project_id);
    if (!canAccessProject(user, project)) return sendJSON(res, 403, { error: 'You do not have access to this task.' });
    const att = db.prepare('SELECT * FROM task_attachments WHERE id = ? AND task_id = ?').get(Number(req.params.attachmentId), task.id);
