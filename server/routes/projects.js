...
async function projectWithMeta(row, currentUserId) {
  const manager = row.manager_id ? await db.prepare('SELECT id, name, email, avatar_color FROM users WHERE id = ?').get(row.manager_id) : null;
  ...
  `).all(row.id);
  const taskStatsRaw = await db.prepare(`
    SELECT status, COUNT(*) as count FROM tasks WHERE project_id = ? GROUP BY status
  `).all(row.id);
  const taskStats = taskStatsRaw.map((t) => ({ status: t.status, count: Number(t.count) }));
...
async function canAccessProject(user, project) {
  ...
  const row = await db.prepare('SELECT id FROM project_members WHERE project_id = ? AND user_id = ?').get(project.id, user.id);
  return !!row;
}
...
function register(router) {
  ...
  router.get('/api/projects', async (req, res) => {
    ...
    if (user.role === 'ADMIN') {
      ...
      rows = await db.prepare('SELECT * FROM projects WHERE manager_id = ? ORDER BY created_at DESC').all(user.id);
    } else {
    ...
    const enriched = await Promise.all(rows.map((r) => projectWithMeta(r, user.id)));
  ...
  router.post('/api/projects', async (req, res) => {
    ...
    if (managerId) {
      const mgr = await db.prepare("SELECT id FROM users WHERE id = ? AND role = 'PROJECT_MANAGER' AND status = 'ACTIVE'").get(managerId);
    ...
    const created = await db.prepare('SELECT * FROM projects WHERE id = ?').get(info.lastInsertRowid);
    sendJSON(res, 201, { project: await projectWithMeta(created, admin.id) });
  });
  ...
  router.get('/api/projects/:id', async (req, res) => {
    const user = await requireAuth(req, res);
    if (!user) return;
    const project = await db.prepare('SELECT * FROM projects WHERE id = ?').get(Number(req.params.id));
    ...
    `).all(project.id);
    sendJSON(res, 200, { project: await projectWithMeta(project, user.id), activity });
  });
  ...
  router.put('/api/projects/:id', async (req, res) => {
    ...
    for (const spec of allowed) {
      ...
      if (body[key] !== undefined) {
        ...
        if (col === 'manager_id') {
          ...
          if (mid) {
            const mgr = await db.prepare("SELECT id FROM users WHERE id = ? AND role = 'PROJECT_MANAGER' AND status='ACTIVE'").get(mid);
            if (!mgr) return sendJSON(res, 400, { error: 'Selected manager is not a valid active Project Manager.' });
          }
    ...
    sendJSON(res, 200, { project: await projectWithMeta(updated, user.id) });
  });
  ...
  router.post('/api/projects/:id/members', async (req, res) => {
    ...
    const member = await db.prepare("SELECT * FROM users WHERE id = ? AND role = 'TEAM_MEMBER' AND status = 'ACTIVE'").get(userId);
    if (!member) return sendJSON(res, 400, { error: 'Selected user is not a valid active Team Member.' });
    const existing = await db.prepare('SELECT id FROM project_members WHERE project_id = ? AND user_id = ?').get(project.id, userId);
    if (existing) return sendJSON(res, 409, { error: 'This user is already on the project team.' });

    await db.prepare('INSERT INTO project_members (project_id, user_id) VALUES (?, ?)').run(project.id, userId);
  ...
  router.delete('/api/projects/:id/members/:userId', async (req, res) => {
    ...
    await db.prepare('DELETE FROM project_members WHERE project_id = ? AND user_id = ?').run(project.id, userId);
    // Unassign that member's tasks in this project
    await db.prepare("UPDATE tasks SET assignee_id = NULL WHERE project_id = ? AND assignee_id = ?").run(project.id, userId);
